...
with col2:
    ...
    with st.container(border=True):
        st.subheader("4. 시간대별 1위 역 변천사")
        st.markdown("- **무엇을 하나요?**: 각 시간대의 '챔피언' (가장 이용객이 많은 역)이 어떻게 변하는지 확인합니다.")
        st.markdown("- **사용 방법**: 시간의 흐름에 따른 1위 역의 변화를 막대그래프로 탐색할 수 있습니다.")
...
with col3:
    ...
    with st.container(border=True):
        st.subheader("6. 시간대별 순위 변화")
        st.markdown("- **무엇을 하나요?**: 시간 흐름에 따른 유동인구 순위 변화를 애니메이션으로 보여줍니다.")
        st.markdown("- **사용 방법**: 분석 기준과 TOP N을 선택하고, 그래프의 **재생(▶️) 버튼**을 눌러 역들의 순위 경쟁을 확인하세요.")
